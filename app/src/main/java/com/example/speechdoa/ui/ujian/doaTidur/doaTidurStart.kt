...
import com.example.speechdoa.databinding.ActivityDoaTidurStartBinding
import com.example.speechdoa.ui.ujian.doaTidur.doaTidurDone

class doaTidurStart : BaseActivity() {

    private lateinit var binding: ActivityDoaTidurStartBinding
    var listDoaTidur = ArrayList<DoaTidurEntity>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDoaTidurStartBinding.inflate(layoutInflater)
        setContentView(binding.root)
        println(this::binding.isInitialized)

        val viewModel = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(doaTidurModel::class.java)
        val doaTidur = viewModel.getDoaTidur()

        listDoaTidur.addAll(doaTidur)
    ...
    private fun startSpeechToText() {
        ...
        speechRecognizer.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(bundle: Bundle) {
                ...
                if (result != null) {
                    println(result[0])
                    if (current_position < listDoaTidur.size - 1) {
                        if (result[0] == listDoaTidur[current_position].doa) {
                            println(result[0])
                            current_position++
                            hasil = "Benar"
                    ...
                    } else if (result[0] == listDoaTidur[current_position].doa) {
                        println(result[0])
                        hasil = "Benar"
                        Toast.makeText(applicationContext, "Benar", Toast.LENGTH_SHORT).show()
                        val intent = Intent(applicationContext, doaTidurDone::class.java)
                        intent.putExtra(doaTidurDone.HASIL, hasil)
                        intent.putExtra(doaTidurDone.SOAL, listDoaTidur[current_position].doa)
                        intent.putExtra(doaTidurDone.JAWABAN, result[0])
