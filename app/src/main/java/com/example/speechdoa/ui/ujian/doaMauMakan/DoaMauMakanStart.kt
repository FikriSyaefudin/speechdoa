...
import com.example.speechdoa.databinding.ActivityDoaMauMakanStartBinding

class DoaMauMakanStart : BaseActivity() {
    private lateinit var binding: ActivityDoaMauMakanStartBinding
    var listDoaMauMakan = ArrayList<DoaMauMakanEntity>()

    var current_position = 0
    var hasil = ""
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityDoaMauMakanStartBinding.inflate(layoutInflater)
        setContentView(binding.root)
        println(this::binding.isInitialized)

        val viewModel = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            DoaMauMakanModel::class.java)
        val doaMakan = viewModel.getDoaMauMakan()

        listDoaMauMakan.addAll(doaMakan)
    ...
    private fun startSpeechToText() {
        ...
        speechRecognizer.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(bundle: Bundle) {
                ...
                if (result != null) {
                    println(result[0])
                    if (current_position < listDoaMauMakan.size - 1) {
                        if (result[0] == listDoaMauMakan[current_position].doa) {
                            println(result[0])
                            current_position++
                            hasil = "Benar"
                    ...
                    } else if (result[0] == listDoaMauMakan[current_position].doa) {
                        println(result[0])
                        hasil = "Benar"
                        Toast.makeText(applicationContext, "Benar", Toast.LENGTH_SHORT).show()
                        val intent = Intent(applicationContext, DoaMauMakanDone::class.java)
                        intent.putExtra(DoaMauMakanDone.HASIL, hasil)
                        intent.putExtra(DoaMauMakanDone.SOAL, listDoaMauMakan[current_position].doa)
