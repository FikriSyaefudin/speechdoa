...
import com.example.speechdoa.databinding.ActivityDoaKetikaHujanLebatStartBinding
...
class DoaKetikaHujanLebatStart : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDoaKetikaHujanLebatStartBinding.inflate(layoutInflater)
        setContentView(binding.root)
        println(this::binding.isInitialized)

        val viewModel = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            DoaKetikaHujanLebatModel::class.java)
        val doaKetikaHujanLebat = viewModel.getDoaKetikaHujanLebat()

        listDoaKetikaHujanLebat.addAll(doaKetikaHujanLebat)
    ...
    private fun startSpeechToText() {
        ...
        speechRecognizer.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(bundle: Bundle) {
                ...
                if (result != null) {
                    println(result[0])
                    if (current_position < listDoaKetikaHujanLebat.size - 1) {
                        if (result[0] == listDoaKetikaHujanLebat[current_position].doa) {
                            println(result[0])
                            current_position++
                            hasil = "Benar"
                    ...
                    } else if (result[0] == listDoaKetikaHujanLebat[current_position].doa) {
                        println(result[0])
                        hasil = "Benar"
                        Toast.makeText(applicationContext, "Benar", Toast.LENGTH_SHORT).show()
                        val intent = Intent(applicationContext, DoaKetikaHujanLebatDone::class.java)
                        intent.putExtra(DoaKetikaHujanLebatDone.HASIL, hasil)
                        intent.putExtra(DoaKetikaHujanLebatDone.SOAL, listDoaKetikaHujanLebat[current_position].doa)
                        intent.putExtra(DoaKetikaHujanLebatDone.JAWABAN, result[0])
